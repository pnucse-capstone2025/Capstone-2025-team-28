...
def generate_single_mpd(output_dir, num_segments, segment_duration=10):
    mpd = ET.Element("MPD", {
        ...
        "profiles": "urn:mpeg:dash:profile:isoff-live:2011",  # live 프로파일로 변경
        "type": "static",
    ...
    # timescale (tick 단위) - 보통 15360 또는 1000 이상 권장
    timescale = 15360
    duration_ticks = segment_duration * timescale  # 한 세그먼트 길이 (ticks 단위)
    ...
    for rep_id, (label, width, height, codec, bandwidth) in enumerate(resolutions):
        ...
        # SegmentTimeline 추가 (r = 반복 횟수 = num_segments-1)
        segment_timeline = ET.SubElement(segment_template, "SegmentTimeline")
        ET.SubElement(segment_timeline, "S", {
            "t": "0",
            "d": str(duration_ticks),
