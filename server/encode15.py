"""
세그먼트 병합하여 하나의 영상으로 만들지 않고.
바로 MP4Box로 넣는 Version
--> 실패
"""
...
def mp4box_dash_multi_res(seg_mp4s_by_res, out_dir, segment_ms=2000):
    """
    MP4Box로 MPEG_DASH 스트리밍용 세그먼트와 MPD(manifest) 파일 생성
    
    Parameters:
    - seg_mp4s_by_res: {해상도:[세그먼트 경로 리스트]} 딕셔너리
    - out_dir: './static/{video}/mp4boㅌ' 폴더 경로
    ...
    for tag, seg_list in seg_mp4s_by_res.items():
        for seg_path in seg_list:
            mp4_args.append(f"{seg_path}#video:name={tag}")
...
def mp4box_dash_multi_res_batched(seg_mp4s_by_res, out_dir, segment_ms=2000, batch_size=20):
    """
    MP4Box로 MPEG-DASH 세그먼트와 MPD(manifest) 생성
    - 세그먼트가 많아 SIGSEGV 발생할 때 배치 단위로 처리
    - 멀티 해상도 지원
    
    Parameters:
    - seg_mp4s_by_res: {해상도_tag: [세그먼트 mp4 경로 리스트]} 딕셔너리
    - out_dir: 출력 폴더
    - segment_ms: 세그먼트 길이(ms)
    - batch_size: 한 번에 MP4Box에 넣는 세그먼트 개수
    """
    ...
    for tag, seg_list in seg_mp4s_by_res.items():
        f.write(f"[{tag}] 총 세그먼트 개수: {len(seg_list)}\n")

        # 세그먼트를 배치 단위로 나눠서 MP4Box 호출
        num_batches = ceil(len(seg_list) / batch_size)
        for batch_idx in range(num_batches):
            batch = seg_list[batch_idx*batch_size:(batch_idx+1)*batch_size]
            mp4_args = [f"{seg}#video:name={tag}" for seg in batch]
            ...
            f.write(f"[{tag}] MP4Box 배치 {batch_idx+1}/{num_batches} 실행: {cmd}\n")
            subprocess.run(cmd, check=True)
